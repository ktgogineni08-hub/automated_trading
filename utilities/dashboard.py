...
class DashboardConnector:
    ...
    def __init__(self, base_url: str = None, api_key: Optional[str] = None):
        ...
        self.verify_certificate_path: Optional[str] = None
        ...
        if disable_tls and self.base_url.startswith("https://"):
            ...
            if cert_used:
                # Requests allows supplying the CA bundle via environment variable.
                # Setting REQUESTS_CA_BUNDLE preserves existing behaviour while
                # letting the session keep verify=True for tests that assert on it.
                os.environ.setdefault("REQUESTS_CA_BUNDLE", str(cert_used))
                self.verify_certificate_path = str(cert_used)
            ...
            self.session.verify = True
            self.verify_certificate_path = None
