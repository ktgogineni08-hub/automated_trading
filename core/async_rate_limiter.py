...
class AsyncRateLimiter:
    ...
    Usage:
        ...
        async def make_api_call():
            async with (await limiter.acquire("api_call")):
                # Make API call
...
def rate_limit(endpoint: str, tokens: float = 1.0):
    ...
    def decorator(func: Callable[..., Coroutine[Any, Any, T]]) -> Callable[..., Coroutine[Any, Any, T]]:
        ...
        async def wrapper(*args, **kwargs) -> T:
            ...
            async with (await limiter.acquire(endpoint, tokens)):
...
async def benchmark_rate_limiter(
    ...
    if use_async:
        ...
        async def make_request():
            async with (await limiter.acquire("benchmark")):
...
if __name__ == "__main__":
    ...
    async def test_burst_handling():
        ...
        contexts = [await limiter.acquire("burst_test") for _ in range(5)]
        await asyncio.gather(*(ctx.__aenter__() for ctx in contexts))
        await asyncio.gather(*(ctx.__aexit__(None, None, None) for ctx in contexts))
