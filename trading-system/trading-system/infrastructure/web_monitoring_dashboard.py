...
class WebMonitoringDashboard:
    ...
    def __init__(self, port: int = 8081, host: Optional[str] = None):
        self.port = port

        env_host = os.getenv("TRADING_DASHBOARD_HOST", "").strip()
        resolved_host = host if host not in (None, "") else (env_host or "127.0.0.1")

        if resolved_host in {"*", "0.0.0.0", "::"}:
            logger.warning(
                "Web dashboard binding to %s; ensure security groups/firewall rules restrict access.",
                resolved_host,
            )
        ...
        logger.info("🌐 Web Monitoring Dashboard initialized on %s:%s", self.host, self.port)
