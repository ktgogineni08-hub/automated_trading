...
class DashboardManager:
    ...
    def check_health(self, timeout: int = 5) -> bool:
        ...
        try:
            ...
            # Only retry with verify=False if it was explicitly disabled via env var
            # Never silently fall back to insecure connections in production
            if not verify_tls:
                logger.debug(
                    "Dashboard TLS verification failed (%s). "
                    "Retrying without verification (DASHBOARD_DISABLE_TLS_VERIFY is set).",
                    ssl_err
                )
                try:
                    # nosec B501 - verify=False only when DASHBOARD_DISABLE_TLS_VERIFY is explicitly set
                    response = requests.get(
                        f"{self.base_url}/health",
                        timeout=timeout,
                ...
                logger.error(
                    "❌ Dashboard TLS verification failed: %s. "
                    "Set DASHBOARD_DISABLE_TLS_VERIFY=true to disable verification (not recommended for production).",
                    ssl_err
                )
                return False
