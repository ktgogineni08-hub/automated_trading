...
class InputSanitizer:
    ...
    def sanitize_symbol(self, symbol: str) -> str:
        """Sanitize a fully qualified trading symbol (e.g., NSE:RELIANCE)."""
        ...
        if ':' in symbol:
            ...
            exchange_clean = self.sanitize_symbol_name(exchange, allow_special=False)
            symbol_clean = self.sanitize_symbol_name(raw_symbol, allow_special=True)
            return f"{exchange_clean}{symbol_clean}"

        return self.sanitize_symbol_name(symbol, allow_special=True)
