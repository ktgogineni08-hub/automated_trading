...
class AdvancedRiskManager:
    ...
    def optimize_risk_parity(
        ...
        Args:
            returns_df: Either a DataFrame of historical returns or a pre-computed
                covariance matrix supplied as a NumPy array.

        Returns:
            Dictionary of weights when a DataFrame is provided; otherwise a NumPy
            array of weights matching the covariance matrix ordering.
        """
        is_cov_matrix_input = isinstance(returns_df, np.ndarray)

        if is_cov_matrix_input:
            cov_matrix = np.asarray(returns_df, dtype=float)
            if cov_matrix.ndim != 2 or cov_matrix.shape[0] == 0:
                return np.array([])
            if cov_matrix.shape[0] != cov_matrix.shape[1]:
                raise ValueError("Covariance matrix must be square.")
            asset_labels = [f"asset_{idx}" for idx in range(cov_matrix.shape[0])]
        else:
            ...
            asset_labels = list(returns_df.columns)

        n_assets = len(asset_labels)
        if n_assets == 0:
            return np.array([]) if is_cov_matrix_input else {}
        ...
        if is_cov_matrix_input:
            return weights
        return {symbol: weight for symbol, weight in zip(asset_labels, weights)}
