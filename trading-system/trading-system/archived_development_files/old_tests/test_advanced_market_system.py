...
except ImportError as exc:  # pragma: no cover - legacy integration script
    pytest.skip(
        f"AdvancedMarketManager unavailable ({exc}); legacy test skipped",
        allow_module_level=True
    )

# Legacy integration suite depends on live market data. Skip under pytest to
# avoid network calls (run manually when needed).
if __name__ != "__main__":  # pragma: no cover - skip only when imported by pytest
    pytest.skip(
        "Legacy AdvancedMarketManager integration scenario requires live data; "
