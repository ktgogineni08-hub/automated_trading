...
class ModelMetadata:
    ...
    artifact_hash: Optional[str] = field(default=None)
...
class MLSignalScorer:
    ...
    def save_model(self, model_id: str):
        ...
        joblib.dump(self.models[model_id], model_path, compress=3)

        artifact_hash = self._compute_file_hash(model_path)
    ...
    def load_model(self, model_id: str):
        ...
        if metadata.artifact_hash and metadata.artifact_hash != file_hash:
            raise ValueError(f"Integrity check failed for model '{model_id}': hash mismatch")

        self.models[model_id] = joblib.load(model_path)
        metadata.artifact_hash = file_hash
        self.metadata[model_id] = metadata
