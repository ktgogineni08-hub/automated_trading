...
class EnhancedRiskManager:
    ...
    def find_correlated_positions(
        self,
        symbol_or_positions: Union[str, Dict[str, float]],
        existing_positions: Optional[Dict[str, float]] = None
    ) -> List[CorrelationRisk]:
        """
        Find positions that are highly correlated

        Args:
            symbol_or_positions: Either a symbol string (with existing_positions)
                                 or a dict of all positions to analyze
            existing_positions: Current positions (only used when symbol_or_positions is a string)

        Returns:
            List of correlation risks
        """
        # Backward compatibility: if first arg is a dict, analyze all positions
        if isinstance(symbol_or_positions, dict):
            all_positions = symbol_or_positions
            risks = []
            symbols = list(all_positions.keys())

            # Find all correlated pairs in the portfolio
            for i, symbol1 in enumerate(symbols):
                for symbol2 in symbols[i+1:]:
                    ...
                    if correlation and correlation >= self.high_correlation_threshold:
                        combined_exposure = all_positions[symbol1] + all_positions[symbol2]
        ...
        # New API: check correlation for a specific symbol
        symbol = symbol_or_positions
        if existing_positions is None:
            raise ValueError("existing_positions required when symbol is a string")
    ...
    def check_correlation_limits(
        ...
        existing_positions: Optional[Dict[str, float]] = None
    ) -> Union[Tuple[bool, Optional[str]], List[CorrelationRisk]]:
        """
        Check if adding position would create excessive correlation risk

        Args:
            symbol_or_positions: Either a symbol string (with new_position_value and existing_positions)
                                or a dict of all positions to analyze for violations
            new_position_value: Value of new position (only used when symbol_or_positions is a string)
            existing_positions: Current positions (only used when symbol_or_positions is a string)

        Returns:
            - (allowed, reason) when checking a new position
            - List of violations when analyzing existing positions
        """
        # Backward compatibility: if first arg is a dict, return list of violations
        if isinstance(symbol_or_positions, dict):
            all_positions = symbol_or_positions
            violations = []

            # Find all correlation violations in the portfolio
            correlated = self.find_correlated_positions(all_positions)

            for risk in correlated:
                exposure_pct = risk.combined_exposure / self.total_capital
                if exposure_pct > self.max_correlation_exposure_pct:
                    violations.append(risk)

            return violations

        # New API: check if adding a position would violate limits
        symbol = symbol_or_positions
        if new_position_value is None or existing_positions is None:
            raise ValueError("new_position_value and existing_positions required when symbol is a string")
    ...
    def calculate_dynamic_position_size(
        ...
        # Ensure position size is positive after adjustments
        if position_size <= 0:
            return 0.0

        # Cap between 0.5% and 5% for valid positions
