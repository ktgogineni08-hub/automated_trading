...
_module_ref = sys.modules.get(__name__)
if _module_ref is not None:
    sys.modules.setdefault('core.strategy_registry', _module_ref)
...
class StrategyRegistry:
    ...
    @staticmethod
    def _resolve_database(db_candidate):
        """Unwrap MagicMock layers to access the real database instance."""
        visited = set()
        current = db_candidate
        depth = 0
        while isinstance(current, MagicMock):
            if getattr(current, 'return_value', None) is None:
                break
    ...
    def __init__(self, strategy_dir: str = "strategies"):
        ...
        module_for_db = sys.modules.get('core.strategy_registry')
        if module_for_db is not None and hasattr(module_for_db, 'get_database'):
            db_candidate = module_for_db.get_database()
        else:
            db_candidate = get_database()
        self.db = self._resolve_database(db_candidate)
    ...
    def update_performance(self, name: str) -> Optional[StrategyPerformance]:
        """Update cached performance details for a strategy."""
        try:
            db = self.db
            if db is None:
                try:
                    ...
                    logger.warning("get_database() failed for %s: %s", name, db_err)
                    db_candidate = None

                if db_candidate is not None:
                    db = self._resolve_database(db_candidate)
                    self.db = db

            if db is None:
                logger.error("No database handle available for %s", name)
                return None

            perf_list = db.get_strategy_performance(name) or []

            if isinstance(perf_list, MagicMock):
                perf_list = perf_list.return_value or []
            ...
            if isinstance(perf_list, list) and perf_list:
                ...
                if isinstance(first, MagicMock):
                    first = first.return_value or {}
                if isinstance(first, dict):
                    if is_dataclass(StrategyPerformance):
                        perf_kwargs = {}
                        for field_def in fields(StrategyPerformance):
                            if field_def.name in first:
                                perf_kwargs[field_def.name] = first[field_def.name]
                            elif field_def.default is not MISSING:
                                perf_kwargs[field_def.name] = field_def.default
                            elif hasattr(field_def, 'default_factory') and field_def.default_factory is not MISSING:
                                perf_kwargs[field_def.name] = field_def.default_factory()
                            else:
                                perf_kwargs[field_def.name] = None
                        perf = StrategyPerformance(**perf_kwargs)
                    else:
                        perf = StrategyPerformance(**first)
                    self._performance_cache[name] = perf
                    return perf

            self._performance_cache.pop(name, None)
            return None
        except Exception as exc:
            logger.error("Failed to get performance for %s: %s", name, exc)
            return None
