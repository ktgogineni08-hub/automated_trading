...
## ✅ Fix 1: Security - Hardcoded Credentials Removed

### Issue
Hardcoded Zerodha API credentials ("<your_api_key>" / "<your_api_secret>") were used as fallbacks, creating a security risk.
...
if not API_KEY or not API_SECRET:
    if trading_mode == 'paper':
        ...
        logger.logger.info("📝 Running in paper trading mode without broker connection")
    else:
...
**Verification**:
- ✅ No hardcoded credentials in any code path
- ✅ Live mode fails immediately without credentials
- ✅ Paper mode continues without credentials
- ✅ Clear error messages guide users to setup_credentials.sh

---

## ✅ Fix 2: Security - Dashboard HTTPS Enforcement
...
**Changes**:
- **Line 11838**: Changed `webbrowser.open("http://localhost:8080")` → `https://localhost:8080`
...
**Verification**:
- ✅ All dashboard URLs use HTTPS consistently
- ✅ Browser opens HTTPS URL
- ✅ Dashboard connector uses HTTPS base_url
- ✅ All log messages show HTTPS URLs

---

## ✅ Fix 3: Signal Strength Directional Asymmetry
...
if signal_type == 'buy' and momentum > 0:
    ...
    strength += momentum_score * 0.25
strength += momentum_score * 0.25  # ❌ UNCONDITIONAL BONUS
```
...
**Verification**:
- ✅ Buy signals only get momentum credit when momentum > 0
- ✅ Sell signals only get momentum credit when momentum < 0
- ✅ No directional asymmetry or double-counting
- ✅ Signal strength properly reflects directional alignment

---

## ✅ Fix 4: Financial Amount Validation
...
**Validation Rules**:
- Initial cash: ₹1,000 to ₹100,000,000
- Current cash: ₹0 to ₹100,000,000 (can be depleted)
- Trade prices: ₹0.01 to ₹1,000,000 per unit
- Trade quantities: 1 to 1,000,000 shares
- All amounts must be finite numbers

**Verification**:
- ✅ Portfolio creation validates initial capital
- ✅ State restoration validates loaded amounts
- ✅ Trade execution validates prices and quantities
- ✅ Invalid amounts raise ValidationError with clear messages
...
**Status**: ✅ All Week 1 fixes implemented and verified
**Date**: 2025-10-08
**Version**: 1.1
