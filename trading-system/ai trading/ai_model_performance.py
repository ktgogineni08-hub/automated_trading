...
class ModelPerformanceTracker:
    ...
    def calculate_directional_accuracy(self, y_true: np.array, y_pred: np.array) -> float:
        ...
        try:
            ...
            self.logger.error(f"Error calculating directional accuracy: {e}")
            return 0.0
    ...
    def evaluate_model(self, symbol: str, model_type: str, y_true: np.array,
        ...
        try:
            ...
            self.logger.error(f"Error evaluating model {symbol}_{model_type}: {e}")
    ...
    def generate_performance_report(self, symbol: str, model_type: str) -> Dict:
        ...
        try:
            ...
            self.logger.error(f"Error generating performance report for {symbol}_{model_type}: {e}")
            return {'error': str(e)}
    ...
    def save_performance_report(self, symbol: str, model_type: str, filepath: str = None):
        ...
        try:
            ...
            if 'error' in report:
                self.logger.error(f"Cannot save report: {report['error']}")
                return False
    ...
    def plot_performance_metrics(self, symbol: str, model_type: str, save_path: str = None):
        ...
        try:
            key = f"{symbol}_{model_type}"
            if key not in self.metrics_history:
                self.logger.error(f"No performance data for {symbol}_{model_type}")
                return False
...
class ModelValidator:
    ...
    def validate_model_stability(self, model, X: np.array, y: np.array,
        ...
        try:
            ...
            self.logger.error(f"Error in model stability validation: {e}")
            return {'error': str(e)}
