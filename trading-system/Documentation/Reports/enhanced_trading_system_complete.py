...
from cross_trade_prevention import OrderFingerprint
from aml_monitor import AMLTransaction
...
# Import security modules for API key encryption
...
from infrastructure.performance import (
    initialize_performance_monitoring, process_signals_optimized,
    can_make_request_optimized, record_request_optimized,
    detect_memory_leaks, force_memory_cleanup, get_performance_report,
    create_optimized_lock, get_optimized_rate_limiter
)
...
# Import enhanced compliance modules
...
from cross_trade_prevention import CrossTradePrevention
from market_abuse_detector import MarketAbuseDetector
...
class TradingLogger:
    ...
    def log_api_call(self, endpoint: str, method: str, status_code: Optional[int] = None, error: Optional[str] = None):
        """Log API calls with sensitive data protection"""
        from infrastructure.security import sanitize_log_message
        if error:
            sanitized_error = sanitize_log_message(str(error))
            self.logger.error(f"API {method} {endpoint} - Status: {status_code} - Error: {sanitized_error}")
        else:
    ...
    # Convenience proxies for standard logging methods
...
def safe_input(prompt: str, default: Optional[str] = None,
    ...
    while retry_count < max_retries:
        try:
            ...
            retry_count += 1
            logger.error(f"Unexpected error in safe_input: {e}", exc_info=True)
            print(f"❌ Error: {e}")

    # Max retries exceeded
    if default is not None:
        logger.warning(f"Max retries ({max_retries}) exceeded, using default: {default}")
...
class DashboardConnector:
    """Enhanced connector with better error handling and secure sessions"""
    ...
    def __init__(self, base_url: str = None):
        ...
        from infrastructure.security import get_session_manager
        self.secure_session_manager = get_session_manager()

        # Configure session with security headers
        self.session.timeout = config.request_timeout
        self.session.headers.update({
            'User-Agent': 'TradingSystem/1.0',
            'X-Session-ID': self._get_or_create_session_id()
        })
    ...
    def _get_or_create_session_id(self) -> str:
        """Get or create a secure session ID"""
        try:
            # Try to get existing session
            session_id = self.secure_session_manager.get_session('dashboard_session')
            if session_id:
                return session_id
            ...
            session_data = {
                ...
                'user_agent': 'TradingSystem/1.0',
                'permissions': ['read', 'write']
            }
            new_session_id = self.secure_session_manager.create_session(session_data)
            logger.info("✅ Created secure dashboard session")
    ...
    def wait_if_needed(self, timeout: float = 10.0) -> bool:
        ...
        while not self.can_make_request():
            if time.time() - start_time > timeout:
                logger.error(f"❌ Rate limit timeout after {timeout}s - API overloaded")
                return False
...
class DataProvider:
    def __init__(self, kite: KiteConnect = None, instruments_map: Dict = None, use_yf_fallback: bool = True):
        ...
        # Use optimized rate limiter for O(1) performance
        self.rate_limiter = get_optimized_rate_limiter()
    ...
    def fetch_with_retry(self, symbol: str, interval: str = "5minute", days: int = 5, max_retries: int = 3) -> pd.DataFrame:
        ...
        for attempt in range(max_retries):
            try:
                # Use optimized rate limiter for O(1) performance
                if not can_make_request_optimized():
...
class UnifiedPortfolio:
    ...
    def __init__(self, initial_cash: float = None, dashboard: DashboardConnector = None, kite: KiteConnect = None, trading_mode: str = 'paper', silent: bool = False):
        ...
        # CRITICAL FIX: Thread safety for concurrent operations with optimized locks
        self._position_lock = create_optimized_lock("portfolio_positions")  # Reentrant lock for nested calls
        self._order_lock = create_optimized_lock("order_placement")  # Separate lock for order placement
        self._cash_lock = create_optimized_lock("cash_operations")  # Lock for cash operations
        ...
        # Enhanced SEBI compliance modules
        self.kyc_manager = KYCManager()
        self.aml_monitor = AMLMonitor()
        self.client_data_protection = ClientDataProtection()
        self.cross_trade_prevention = CrossTradePrevention()
    ...
    def save_state_to_files(self):
        """
        Save current portfolio state to files for dashboard integration with encryption
        ...
        def _resolve_state_path(candidate: Path) -> Path:
            """Resolve candidate path ensuring it stays within the state directory with enhanced security."""
            try:
                # Use security module for path validation
                validated_path = validate_secure_path(str(candidate))
        ...
        for attempt in range(max_retries):
            try:
                ...
                with self._position_lock:
                    ...
                    for symbol, pos in self.positions.items():
                        try:
                            ...
                            logger.warning(f"⚠️ Skipping invalid position {symbol}: {e}")
                            continue

                # Make entire portfolio state JSON serializable
                portfolio_state = self._make_json_serializable(portfolio_state)

                # Use encrypted state storage for security
                try:
                    # Save encrypted state
                    success = save_encrypted_state(portfolio_state, 'shared_portfolio_state.json')
                    if not success:
                        # Fallback to atomic write if encryption fails
                        _write_with_timeout(
                            lambda: atomic_write_json(shared_state_path, portfolio_state, create_backup=True),
                            "Shared state write (fallback)"
                        )
                    else:
                        logger.info("🔒 Portfolio state encrypted and saved securely")
                except TimeoutError as e:
                    logger.error("❌ Shared state write timeout: %s", e, exc_info=True)
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                    raise
                except Exception as e:
                    logger.error("❌ Failed to write shared state file: %s", e, exc_info=True)
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                    raise
                ...
                try:
                    # Save encrypted state
                    success = save_encrypted_state(current_state, 'current_state.json')
                    if not success:
                        # Fallback to atomic write if encryption fails
                        _write_with_timeout(
                            lambda: atomic_write_json(current_state_path, current_state, create_backup=True),
                            "Current state write (fallback)"
                        )
                    else:
                        logger.info("🔒 Current state encrypted and saved securely")
                except TimeoutError as e:
                    logger.error("❌ Current state write timeout: %s", e, exc_info=True)
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                    raise
                except Exception as e:
                    logger.error("❌ Failed to write current state file: %s", e, exc_info=True)
                    if attempt < max_retries - 1:
                        time.sleep(retry_delay)
                        continue
                    raise

                logger.debug(f"✅ State saved successfully (attempt {attempt + 1}/{max_retries}): {len(self.positions)} positions, cash: ₹{self.cash:,.2f}")
                return  # Success - exit retry loop

            except Exception as e:
                logger.error(
                    "❌ Error saving state (attempt %d/%d): %s",
                    attempt + 1,
                    max_retries,
                    e,
                    exc_info=True
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error("❌ Failed to save state after %d attempts", max_retries)
                    try:
                        ...
                        logger.warning(
                            "⚠️ State saved to backup files: %s, %s",
                            backup_shared,
                            backup_current
                        )
                    except Exception as backup_error:
                        logger.critical(
                            "🚨 Failed to write backup state files: %s",
                            backup_error,
                            exc_info=True
                        )
        ...
        for symbol, pos in positions_snapshot.items():
            ...
            if price_map and symbol in price_map:
                ...
                if fetched_price is not None and fetched_price > 0:
                    ...
                    logger.debug(f"📊 Dashboard update: {symbol} using fetched price ₹{current_price:.2f}")
                else:
                    logger.warning(f"⚠️ Dashboard update: {symbol} fetched invalid price {fetched_price}, using entry price ₹{pos['entry_price']:.2f}")
            else:
                logger.debug(f"📊 Dashboard update: {symbol} using entry price ₹{pos['entry_price']:.2f} (no price_map)")
    ...
    def save_daily_trades(self, trading_day: str = None) -> Dict[str, any]:
        ...
        try:
            ...
            logger.info(
                f"💾 Saved {len(daily_trades)} trades for {trading_day} → {file_path}"
            )
    ...
    def execute_trade(self, symbol: str, shares: int, price: float, side: str, timestamp: datetime = None, confidence: float = 0.5, sector: str = None, atr: float = None, allow_immediate_sell: bool = False, strategy: str = None) -> Optional[Dict]:
        ...
        if side == "buy":
            # ENHANCED SEBI COMPLIANCE VALIDATION
            try:
                # 1. Basic validation
                ...
                if hasattr(self, 'kyc_manager'):
                    is_kyc_compliant, kyc_reason = self.kyc_manager.check_kyc_compliance("CLIENT_001")  # Use actual client ID
                    if not is_kyc_compliant:
                        logger.error(f"KYC Compliance Failed: {kyc_reason}")
                        return None

                # 3. AML Risk Assessment
                if hasattr(self, 'aml_monitor'):
                    client_risk_score = self.aml_monitor.get_client_risk_score("CLIENT_001")  # Use actual client ID
                    if client_risk_score > 80:  # High risk threshold
                        logger.error(f"AML Risk Too High: Score {client_risk_score}/100")
                        return None

                # 4. Cross Trade Prevention
                if hasattr(self, 'cross_trade_prevention'):
                    order_fingerprint = OrderFingerprint(
                        client_id="CLIENT_001",  # Use actual client ID
                        symbol=symbol,
                        side=side,
                        price=price,
                        quantity=shares,
                ...
                # 5. Market Abuse Check
                if hasattr(self, 'market_abuse_detector'):
                    trade_data = {
                        'client_id': "CLIENT_001",  # Use actual client ID
                        'symbol': symbol,
                        'price': price,
            ...
            if hasattr(self, 'client_data_protection'):
                self.client_data_protection.log_data_access(
                    user_id="CLIENT_001",  # Use actual client ID
                    action="trade",
            ...
            if hasattr(self, 'aml_monitor'):
                aml_transaction = AMLTransaction(
                    transaction_id=trade_result.get('trade_id', ''),
                    client_id="CLIENT_001",  # Use actual client ID
                    amount=execution_price * shares,
                    transaction_type="trade",
                    timestamp=timestamp.isoformat()
                )
                self.aml_monitor.record_transaction(aml_transaction)
            ...
            if hasattr(self, 'cross_trade_prevention'):
                ...
                self.cross_trade_prevention.record_order(order_fingerprint)
            ...
            if hasattr(self, 'market_abuse_detector'):
                ...
                self.market_abuse_detector.record_trade(trade_data)
...
class UnifiedTradingSystem:
    ...
    def scan_batch(self, batch: List[str], interval: str, batch_num: int, total_batches: int) -> Tuple[Dict, Dict]:
        """Scan a batch of symbols for signals with optimized performance"""
        signals = {}
        prices = {}
        logger.info(f"  Batch {batch_num}/{total_batches}: {', '.join(batch[:3])}...")

        try:
            # Use optimized signal processing for better performance
            optimized_results = process_signals_optimized(batch, self.dp, self.strategies)

            # Process results from optimized processor
            for symbol, result in optimized_results.items():
                try:
                    # Get current price for the symbol
                    if hasattr(self.dp, 'price_cache'):
                        current_price = self.dp.price_cache.get(symbol)
                    else:
                        # Fallback to fetching price
                        df = self.dp.fetch_with_retry(symbol, interval=interval, days=1)
                        current_price = safe_float_conversion(df["close"].iloc[-1]) if not df.empty else 0
                    ...
                    # Apply trend filter and confidence threshold (same logic as before)
                    if self._should_include_signal(symbol, result, current_price, is_exit_signal):
                        # Add ATR calculation for compatibility
                        df = self.dp.fetch_with_retry(symbol, interval=interval, days=5)
                        if not df.empty:
                            result['atr'] = self._calculate_atr(df)
                        result['last_close'] = current_price
                    ...
                    logger.error(f"Error processing optimized result for {symbol}: {e}")
                    continue

        except Exception as e:
            logger.error(f"Error in optimized signal processing: {e}")
            # Fallback to original processing method
            logger.info("Falling back to original signal processing method")
            return self._scan_batch_fallback(batch, interval, batch_num, total_batches)

        return signals, prices

    def _should_include_signal(self, symbol: str, result: Dict, current_price: float, is_exit_signal: bool) -> bool:
        """Determine if signal should be included based on filters"""
        try:
            ...
            if trend_filter_enabled and not (self.trading_mode == 'paper' and trading_profile == 'Aggressive') and not is_exit_signal:
                ...
                df = self.dp.fetch_with_retry(symbol, interval="5minute", days=5)
                if not df.empty:
                    ...
                    if result['action'] == 'sell' and not downtrend:
                        logger.info(f"    {symbol}: Sell entry blocked - not in downtrend (new position only)")
                        return False
                    if result['action'] == 'buy' and not uptrend:  # STRICTER: Always require uptrend for buys
            ...
            if not is_exit_signal and result['confidence'] < min_confidence:
                logger.info(f"    {symbol}: Entry signal confidence {result['confidence']:.1%} below threshold {min_confidence:.1%} (new position only)")
                return False

            return result['action'] != 'hold'

        except Exception as e:
            logger.error(f"Error checking signal filters for {symbol}: {e}")
            return False

    def _scan_batch_fallback(self, batch: List[str], interval: str, batch_num: int, total_batches: int) -> Tuple[Dict, Dict]:
        """Fallback signal processing method (original implementation)"""
        signals = {}
        prices = {}
        ...
        for symbol in batch:
            try:
                ...
                if aggregated['action'] != 'hold':
                    ...
                    # Apply same filters as in _should_include_signal
                    if self._should_include_signal(symbol, aggregated, current_price, is_exit_signal):
                ...
                try:
                    ...
                    if iteration % 50 == 0:  # Check every 50 iterations
                        ...
                        if memory_analysis.get('leak_detected'):
                            logger.warning(f"🔍 Memory leak detected: {memory_analysis}")
                    ...
                    # CRITICAL FIX: Log bypass status for transparency
                    if bypass_market_hours:
                        logger.warning("⚠️ MARKET HOURS BYPASS ENABLED")
                        logger.warning("⚠️ System will trade outside normal market hours")
                        logger.warning("⚠️ This should only be used for testing purposes")
                        logger.warning("⚠️ Production systems should have bypass_market_hours: false")
                    else:
                        logger.debug("✅ Market hours bypass disabled - respecting normal trading hours")
                    ...
                    if self.trading_mode != 'backtest' and should_stop_trading:
                        ...
                        # CRITICAL FIX: Enhanced bypass validation with additional safety checks
                        if bypass_market_hours:
                            # Additional safety check: Only allow bypass for paper trading, never for live trading
                            if self.trading_mode == 'live':
                                logger.error("🚫 BYPASS BLOCKED: Cannot bypass market hours in LIVE trading mode!")
                                logger.error("🚫 This is a critical safety feature to prevent real money losses")
                                # STOP TRADING - markets are closed and bypass is not allowed for live trading
                            else:
                                ...
                                logger.warning("⚠️ Consider disabling bypass for production use")
                                logger.warning("⚠️ Market data may be outdated and trading signals unreliable")
                    ...
                    if option:
                        ...
                        if i % 4 == 0:
                            try:
                                ...
                                logger.debug(f"Live data fetch failed for {inst['tradingsymbol']}: {e}")
        ...
        try:
            ...
            if option_quote and quote_symbol in option_quote:
                ...
                if option.last_price > 0 and option.last_price != old_price:
                    logger.debug(f"✅ Updated {option.symbol} with live data: price=₹{option.last_price:.2f}, oi={option.open_interest}")
                    return True
                else:
                    logger.debug(f"📊 {option.symbol} quote received but no price change: ₹{option.last_price:.2f}")
                    return False
            else:
                logger.debug(f"❌ No quote data for {option.symbol}")
                return False
        except Exception as e:
            logger.debug(f"❌ Could not fetch live data for {option.symbol}: {e}")
            return False
    ...
    def execute_optimal_strategy(self, index_symbol: str, capital: float = 100000, portfolio: UnifiedPortfolio = None) -> Dict:
        ...
        try:
            ...
            if not execution_result['success']:
                logger.warning(f"Primary strategy {recommendation['strategy']} failed: {execution_result.get('error')}")
                ...
                for fallback_strategy in fallback_strategies:
                    ...
                    if fallback_details:
                        ...
                        if fallback_result['success']:
                            ...
                            logger.warning(f"Fallback strategy {fallback_strategy} also failed: {fallback_result.get('error')}")
...
class FNOTerminal:
    ...
    def _cleanup_fno_state_files(self) -> None:
        """Remove persisted F&O state files so a fresh session starts clean."""
        state_files = [
    ...
    def _reset_fno_session(self) -> float:
        """
        Reinitialize the F&O portfolio when the user opts to start fresh.

        Returns:
            The cash balance of the newly created clean portfolio.
        """
        dashboard = getattr(self.portfolio, 'dashboard', None)
        trading_mode = getattr(self.portfolio, 'trading_mode', 'paper')
        initial_cash = getattr(self.portfolio, 'initial_cash', config.initial_capital)

        logger.info("🔄 Resetting F&O trading session to a clean state")
        self._cleanup_fno_state_files()
        ...
        self.intelligent_selector = IntelligentFNOStrategySelector(
            kite=self.kite,
            portfolio=self.portfolio
        )
        self.iteration = 0
        ...
        try:
            ...
            logger.warning(f"⚠️ Unable to persist fresh F&O state: {exc}")
    ...
    def run_continuous_fno_monitoring(self):
        ...
        if state_loaded:
            ...
            if resume_choice in ['n', 'no']:
                ...
                new_cash = self._reset_fno_session()
                print(f"✅ Portfolio reset to ₹{new_cash:,.0f}")
                state_loaded = False
        ...
        if not state_loaded:
            try:
                ...
                print("\n❌ Input cancelled, using default values")
                min_confidence = 0.60
                check_interval = 300
                max_positions = 5
        ...
        while True:
            try:
                cap_input = input(
                    f"Capital per trade (₹) [200000] (max: {max_allowed_capital:,.0f}): "
                ).strip() or "200000"
                ...
                if capital_per_trade > max_allowed_capital:
                    print(f"❌ Exceeds 25% of portfolio (₹{max_allowed_capital:,.0f})")
        ...
        try:
            while True:
                ...
                if iteration % 10 == 0:
                    logger.info("🔄 Periodic position sync from Kite broker...")
                    sync_result = self.portfolio.sync_positions_from_kite()
                ...
                if timedelta(minutes=-60) < time_to_close <= timedelta(minutes=5):
                    if getattr(self, 'last_archive_day', None) != current_day:
                        try:
                            logger.info("📦 Starting F&O end-of-day trade archival...")
                            archive_result = self.portfolio.archive_end_of_day_trades()
                            if archive_result['status'] == 'success':
                                ...
                                logger.error(
                                    f"❌ Trade archival failed: {archive_result.get('errors')}"
                                )
                        except Exception as archive_error:
                            logger.error(f"❌ CRITICAL: Trade archival crashed: {archive_error}")
                            traceback.print_exc()
                ...
                if current_positions > 0:
                    ...
                    logger.info(f"🔍 MONITORING: {current_positions} positions in portfolio")

                    # Get current option symbols from positions
                    option_symbols = [symbol for symbol in self.portfolio.positions.keys()]
                    logger.info(f"🔍 Position symbols: {', '.join(option_symbols)}")

                    # Fetch current market prices ONCE
                    logger.info(f"💰 Fetching current prices for {len(option_symbols)} positions...")
                    current_prices = self.data_provider.get_current_option_prices(option_symbols)
                    logger.info(f"💰 Received prices for {len(current_prices)}/{len(option_symbols)} positions")
                    ...
                    for sym in option_symbols:
                        ...
                        if curr_price > 0:
                            ...
                            logger.info(f"📈 {sym}: entry=₹{entry_price:.2f}, current=₹{curr_price:.2f}, shares={shares}, P&L=₹{pnl:,.2f}")
                        else:
                            logger.warning(f"⚠️ {sym}: No current price received!")

                    # Monitor positions for exit signals
                    logger.info("🔎 Calling monitor_positions()...")
                    position_analysis = self.portfolio.monitor_positions(current_prices)
                    logger.info(f"🔎 monitor_positions() returned {len(position_analysis)} analyses")

                    # Log what monitor_positions found
                    for sym, analysis in position_analysis.items():
                        if analysis['should_exit']:
                            logger.info(f"🚨 {sym} SHOULD EXIT: {analysis['exit_reason']} (P&L: ₹{analysis['unrealized_pnl']:,.2f})")
                        else:
                            logger.info(f"✋ {sym} should NOT exit (P&L: ₹{analysis['unrealized_pnl']:,.2f})")

                    # Execute exits for positions that meet criteria
                    logger.info("⚡ Calling execute_position_exits()...")
                    exit_results = self.portfolio.execute_position_exits(position_analysis)
                    logger.info(f"⚡ execute_position_exits() returned {len(exit_results)} results")
    ...
    def run_auto_strategy_selection(self):
        ...
        try:
            ...
            capital = get_validated_float(
                "Available capital for this trade (₹) [1000000]: ",
                min_val=10000.0,
    ...
    def run_continuous_trading(self, index_symbol: str, capital: float, depth: int = 0, max_depth: int = 25):
        ...
        if depth >= max_depth:
            logger.error(
                "🚨 Continuous trading depth limit reached for %s (depth=%d)",
                index_symbol,
                depth
            )
            print("⚠️ Maximum continuous trading depth reached. Stopping to prevent recursion overflow.")
            return
...
def start_dashboard() -> Optional[subprocess.Popen]:
    ...
    try:
        dashboard_script = Path("enhanced_dashboard_server.py")
        if not dashboard_script.exists():
            logger.warning("Dashboard server file not found; skipping startup.")
            return None
        ...
        if not api_key:
            ...
            print(f"✅ Generated dashboard API key for this session: {api_key}")
            print("   Tip: export DASHBOARD_API_KEY to reuse the same key next time.")

        dashboard_url = os.environ.get("DASHBOARD_BASE_URL", "http://localhost:8080")
        os.environ["DASHBOARD_BASE_URL"] = dashboard_url
        use_https = dashboard_url.startswith("https://") or os.environ.get("DASHBOARD_USE_HTTPS") == "1"

        env = os.environ.copy()
        env["DASHBOARD_API_KEY"] = api_key
        env["DASHBOARD_BASE_URL"] = dashboard_url
        ...
        launch_url = f"{dashboard_url}?api_key={api_key}"
        try:
            webbrowser.open(launch_url)
            logger.info(f"✅ Dashboard started at: {dashboard_url}")
            logger.info("ℹ️  If the browser does not open automatically, visit the URL above.")
        except Exception:
            logger.info(f"Dashboard started at: {dashboard_url} (open manually)")
            logger.info(f"🔑 Append '?api_key={api_key}' to the URL on first launch.")
...
def run_trading_system_directly(trading_mode: str, config: Dict) -> None:
    ...
    logger.info(f"🎯 STARTING {trading_mode.upper()} TRADING SYSTEM")
    logger.info("="*60)
    ...
    dashboard = DashboardConnector(base_url=dashboard_url, api_key=dashboard_api_key)
    if not dashboard.ensure_connection(force=True):
        time.sleep(2)
        dashboard.ensure_connection(force=True)
    ...
    # Try to decrypt stored API keys if not in environment
    if not API_KEY or not API_SECRET:
        logger.info("🔑 No API keys in environment, checking encrypted storage...")
        API_KEY = decrypt_api_key('zerodha_api_key')
        API_SECRET = decrypt_api_key('zerodha_api_secret')

    # If not in environment or encrypted storage, prompt user to enter them
    if not API_KEY or not API_SECRET:
        if trading_mode in ['paper', 'backtest']:
            ...
            response = input(f"\n📝 Enter API credentials for {mode_label}? (y/n, default=n): ").strip().lower()
            ...
            if response == 'y':
                ...
                if API_KEY and API_SECRET:
                    # Encrypt and store API keys for future use
                    try:
                        encrypt_api_key(API_KEY, 'zerodha_api_key')
                        encrypt_api_key(API_SECRET, 'zerodha_api_secret')
                        logger.info("✅ API credentials encrypted and stored securely")
                    except Exception as e:
                        logger.warning(f"⚠️ Failed to encrypt API keys: {e}")

                    logger.info("✅ API credentials entered manually")
                else:
                    logger.info(f"ℹ️  Continuing without credentials - {mode_label.lower()} mode")
                    API_KEY = None
                    API_SECRET = None
            else:
                logger.info(f"ℹ️  {mode_label} mode - running without broker connection")
                API_KEY = None
                API_SECRET = None
        else:
            # LIVE mode: credentials required
            logger.warning("⚠️  No credentials found in environment")
            ...
            response = input("\n🔐 Enter API credentials now? (y/n): ").strip().lower()
            ...
            if response == 'y':
                ...
                logger.error("❌ Cannot proceed with LIVE trading without credentials")
                raise ConfigurationError("Missing Zerodha API credentials for live trading")
        ...
        try:
            ...
            if kite:
                ...
                if trading_mode == 'live':
                    ...
                    logger.info(f"👤 Account: {profile.get('user_name')}")
                    logger.info(f"💰 Available Cash: ₹{available_cash:,.2f}")

                    max_capital = config.get('max_capital', 100000)
                    if available_cash < max_capital:
                        logger.warning(f"Warning: Available cash (₹{available_cash:,.2f}) < Max capital (₹{max_capital:,.2f})")
            else:
                if trading_mode == 'live':
                    logger.error("❌ Zerodha authentication failed - cannot proceed without Kite API")
                    raise ValueError("Kite API authentication required for live trading")
                else:
                    logger.warning("⚠️  Broker connection failed - continuing in paper mode")

        except Exception as e:
            if trading_mode == 'live':
                logger.error(f"Authentication error: {e}")
                logger.error("❌ System requires live Kite data - no external fallbacks available")
                raise ValueError("Kite API authentication required for live trading")
            else:
                logger.warning(f"⚠️  Broker connection error: {e} - continuing in paper mode")
    else:
        # No credentials - paper mode only
        if trading_mode == 'paper':
            logger.info("📝 Running in paper trading mode without broker connection")
        else:
    ...
    if trading_mode == 'live':
        ...
        logger.info(f"✅ Added {len(conservative_symbols)} conservative symbols for live trading")
    else:
        # Full symbol set for paper/backtest
        system.add_symbols(NIFTY_50_SYMBOLS[:30])
        logger.info("✅ Added top 30 NIFTY stocks")
    ...
    logger.info(f"✅ Symbols: {len(system.symbols)} stocks")
    logger.info(f"✅ Max Positions: {system.max_positions}")
    logger.info(f"✅ Check Interval: {check_interval}s")
    logger.info(f"✅ Dashboard: {'Connected' if dashboard.is_connected else 'Offline'}")

    if dashboard.is_connected:
        logger.info("📊 Monitor at: http://localhost:8080")
    ...
    logger.info(mode_warnings.get(trading_mode, ''))
    ...
    try:
        ...
        logger.info(f"🛑 {trading_mode.upper()} trading stopped by user")

        # Save trades before exit
        try:
            if hasattr(system, 'portfolio') and system.portfolio:
                logger.info("💾 Saving current portfolio state...")
        ...
        if system.portfolio.positions:
            logger.warning(f"⚠️ You have {len(system.portfolio.positions)} open positions:")
            for symbol, pos in system.portfolio.positions.items():
                logger.warning(f"   {symbol}: {pos['shares']} shares @ ₹{pos['entry_price']:.2f}")

            if trading_mode == 'live':
                logger.info("💡 Consider closing positions manually if needed")

def run_paper_trading() -> None:
    """Run paper trading mode"""
    logger.info("📝 PAPER TRADING MODE")
    logger.info("="*50)
    logger.info("✅ Safe simulation with real market data")
    logger.info("✅ No financial risk")
    logger.info("✅ Perfect for testing strategies")
    logger.info("✅ Virtual capital: ₹10,00,000")
    ...
    if profile == '3':
        ...
        logger.info("💰 Profit-Focused Profile Selected:")
        logger.info(f"   • Min Confidence: {min_confidence} (Balanced for opportunities)")
        logger.info(f"   • Top N Signals: {top_n} (Focus on highest quality)")
        logger.info(f"   • Max Positions: {max_positions} (Moderate for returns)")
        logger.info(f"   • Stop Loss: {stop_loss_pct:.1%} (F&O Risk Control)")
        logger.info(f"   • Take Profit: {take_profit_pct:.0%} (Higher targets)")
        logger.info("   • Signal Agreement: 30% (vs 40% for other profiles)")
        logger.info("   • Enhanced exit strategy with volatility adjustment")
    elif profile == '2':
        ...
        logger.info("⚖️ Balanced Profile Selected:")
        logger.info(f"   • Min Confidence: {min_confidence}")
        logger.info(f"   • Top N Signals: {top_n}")
        logger.info(f"   • Max Positions: {max_positions}")
        logger.info(f"   • Stop Loss: {stop_loss_pct:.0%}")
        logger.info(f"   • Take Profit: {take_profit_pct:.0%}")
    else:
        ...
        logger.info("🎯 Quality Profile Selected:")
        logger.info(f"   • Min Confidence: {min_confidence}")
        logger.info(f"   • Top N Signals: {top_n}")
        logger.info(f"   • Max Positions: {max_positions}")
        logger.info(f"   • Stop Loss: {stop_loss_pct:.0%}")
    ...
    config = {
        ...
        'bypass_market_hours': False,  # CRITICAL FIX #6: Respect market hours by default - NEVER enable for live trading
    ...
    try:
        ...
        logger.info("💾 Trades saved automatically during execution")
    except Exception as e:
...
def run_live_trading() -> None:
    ...
    logger.info("="*50)
    logger.warning("⚠️  WARNING: REAL MONEY WILL BE USED!")
    logger.warning("⚠️  Actual trades will be executed!")
    logger.warning("⚠️  You can lose real money!")
    ...
    if max_position > 0.25:
        logger.warning(f"⚠️ Position size {max_position:.1%} exceeds recommended maximum of 25%")
...
def run_backtesting() -> None:
    ...
    logger.info("="*50)
    logger.info("⚡ Fast strategy testing on historical data")
    ...
    try:
        ...
        logger.info("💾 Backtest data saved automatically during execution")
    except Exception as e:
        logger.error(f"Error: {e}")
    finally:
        if dashboard_process:
...
def main() -> None:
    ...
    if API_KEY and API_SECRET:
        ...
        try:
            ...
            if kite:
                ...
                logger.error("❌ Zerodha authentication failed - F&O system requires Kite API")
                logger.error("❌ Cannot proceed with F&O trading without live data")
                raise ValueError("Kite API authentication required for F&O trading")

        except Exception as e:
            logger.error(f"Authentication error: {e}")
            logger.error("❌ F&O system requires live Kite data - no external fallbacks available")
            raise ValueError("Kite API authentication required for F&O trading")
    ...
    if verbose:
        logger.info("🎯 ENHANCED NIFTY 50 TRADING SYSTEM")
        logger.info("="*60)
        logger.info("🚀 All improvements integrated for maximum profits!")
        logger.info("📊 Dashboard integration with real-time monitoring")
        logger.info("🔧 Enhanced token management and error handling")
        logger.info("="*60)
        logger.info("")
        logger.info("Select Trading Type:")
        logger.info("1. 📈 NIFTY 50 Trading")
        logger.info("2. 🎯 F&O Trading (Futures & Options)")
        logger.info("="*60)
    else:
        ...
        try:
            ...
            if trading_type == "1":
                ...
                if dashboard.is_connected:
                    ...
                    print(f"📊 Monitor F&O trades at: {dashboard_url}")
                    try:
...
def test_aggressive_profile() -> bool:
    ...
    logger.info("🧪 Testing Profit-Focused Profile Implementation")
    logger.info("="*50)
    ...
    logger.info("✅ Profit-focused profile configuration:")
    logger.info(f"   • Trading Profile: {config['trading_profile']}")
    logger.info(f"   • Min Confidence: {config['min_confidence']}")
    logger.info(f"   • Top N Signals: {config['top_n']}")
    logger.info(f"   • Max Positions: {config['max_positions']}")
    logger.info(f"   • Stop Loss: {config['stop_loss_pct']:.0%}")
    logger.info(f"   • Take Profit: {config['take_profit_pct']:.0%}")
    ...
    try:
        ...
        logger.info("✅ UnifiedTradingSystem initialized with profit-focused profile")
        logger.info(f"   • Max Positions: {system.max_positions}")
        logger.info(f"   • Portfolio Min Position Size: {system.portfolio.min_position_size}")
        logger.info(f"   • Portfolio Max Position Size: {system.portfolio.max_position_size}")
        logger.info(f"   • Portfolio Risk Per Trade: {system.portfolio.risk_per_trade_pct}")
        ...
        logger.info("✅ Signal processing settings:")
        logger.info(f"   • Min Confidence: {min_conf}")
        logger.info(f"   • Top N: {top_n}")

        logger.info("🎉 Profit-focused profile test completed successfully!")
        return True

    except Exception as e:
        logger.error(f"❌ Error testing profit-focused profile: {e}")
        return False

def test_paper_trading_with_signals() -> bool:
    """Test paper trading with simulated signals to verify trading logic"""
    logger.info("🧪 Testing Paper Trading with Simulated Signals")
    logger.info("="*50)
    ...
    config = {
        ...
        'bypass_market_hours': False  # CRITICAL FIX: Never enable bypass for test functions - use only for debugging
    }

    logger.info("✅ Test configuration:")
    logger.info(f"   • Trading Profile: {config['trading_profile']}")
    logger.info(f"   • Min Confidence: {config['min_confidence']} (lowered for testing)")
    logger.info(f"   • Top N Signals: {config['top_n']}")
    logger.info(f"   • Max Positions: {config['max_positions']}")
    logger.info(f"   • Bypass Market Hours: {config.get('bypass_market_hours', False)}")
    ...
    try:
        ...
        logger.info(f"✅ Added {len(test_symbols)} test symbols")

        # Test signal generation with mock data
        logger.info("🔍 Testing signal generation...")
        signals, prices = system.scan_batch(test_symbols[:3], "5minute", 1, 1)

        logger.info(f"✅ Generated {len(signals)} signals from {len(prices)} price updates")

        if signals:
            logger.info("📊 Sample signals:")
            for symbol, signal in list(signals.items())[:3]:
                logger.info(f"   • {symbol}: {signal['action'].upper()} @ {signal['confidence']:.1%}")

        logger.info("🎉 Signal testing completed successfully!")
        return True

    except Exception as e:
        logger.error(f"❌ Error testing paper trading: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def run_performance_test() -> None:
    ...
    logger.info("🚀 PERFORMANCE TEST - Profit-Focused Settings")
    logger.info("="*60)
    logger.info("Testing profit-focused parameters for maximum returns:")
    logger.info("• Risk per trade: 1.8% (optimized)")
    logger.info("• Stop loss: 2% (tighter)")
    logger.info("• Take profit: 30% (higher targets)")
    logger.info("• Min confidence: 50% (balanced for opportunities)")
    logger.info("• New Momentum Strategy added")
    logger.info("• Profit-focused profile: 18 positions max")
    logger.info("• Focus on top 3 highest-confidence signals")
    logger.info("="*60)
    ...
    logger.info("📊 Starting performance test with profit-focused settings...")
    logger.info("💰 Initial Capital: ₹10,00,000")
    logger.info("📈 Testing 30 days of historical data")
    logger.info("💰 Using Profit-Focused profile with new Momentum strategy")
    ...
    try:
        ...
        logger.info("🛑 Performance test stopped by user")
        logger.info("💾 Performance data saved automatically during execution")
    except Exception as e:
